...
1. 每个实际存在的 xingbuild task（含项目 automation task）都必须始终维护自己的状态；开始/恢复移除 `✅`/`⚠️`，由 Codex 动态圆圈表示进行中；完成或无需 Xing 处理改为 `✅`；待 Xing 决策改为 `⚠️`。进行中无标题前缀不是无状态，而是使用 Codex 原生状态。
2. 标题只能有一个状态前缀；更新时保留职责别名和原有语义，不把整段 delegation、版本事实或错误摘要塞进标题。
3. `⚠️` 只能表示“现在不能继续且需要 Xing 决策/授权/信息”，不能用于普通风险、未来建议、外部观察或等待其他 task 的正常回传；技术/网络/其他 task 阻断若不需要 Xing 决策，不使用 `⚠️`，在检查点写明阻断类型和恢复 owner。
...
| 技术/网络/其他 task 阻断，但不需要 Xing 决策 | 不使用 `⚠️`；必要时以 `✅` 表示当前无需 Xing 处理 | `阻断类型=technical|network|external-task`、证据、恢复 owner |
