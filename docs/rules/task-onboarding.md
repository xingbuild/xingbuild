...
| 状态 | 标记 | 使用条件 |
| --- | --- | --- |
| 正在执行或可继续 | 无前缀 | task 正在处理，或存在建议/外部状态但 task 仍可安全推进 |
| Xing 无需处理 | `✅` | task 已完成，或当前没有需要 Xing 决策、授权、选择或补充信息的事项 |
| 阻断待 Xing | `⚠️` | task 因缺少 Xing 的确认、授权、选择或必要信息而无法继续 |

强制规则：

1. 每个实际存在的 xingbuild task 都必须维护自己的标题状态；进行中立即移除 `✅`/`⚠️`，完成或阻断时立即更新。
2. 标题只能有一个状态前缀；更新时保留职责别名和原有语义，不把整段 delegation、版本事实或错误摘要塞进标题。
3. `⚠️` 只能表示“现在不能继续”，不能用于普通风险、未来建议、外部观察或等待其他 task 的正常回传。
...
| 事件 | task 标题动作 | 回传必须补充 |
| --- | --- | --- |
| 开始/恢复执行 | 立即移除 `✅`/`⚠️` | 当前目标与下一里程碑 |
| 仍可继续处理 | 保持无前缀 | 已完成/未完成与下一动作 |
| 完成，或当前无需 Xing 决策 | 添加 `✅` | 已完成证据、未执行动作、下一触发条件 |
| 缺少 Xing 决策/授权/必要信息，无法继续 | 添加 `⚠️` | `阻断类型=user-decision`、缺什么、恢复条件 |
| 技术/网络/其他 task 阻断，但不需要 Xing 决策 | 不使用 `⚠️`；停止后可用 `✅` 表示无需 Xing 处理 | `阻断类型=technical|network|external-task`、证据、恢复 owner |

每个 task 必须在以下节点执行一次自检并更新标题：首次开始、恢复、交接后、用户新指令改变范围、阻断、最终回传。源 task 发送一次交接后必须结束当前回合，不通过轮询维持“进行中”；目标 task 在里程碑回传时自行更新标题。状态前缀只能有一个，不能叠加或保留旧状态。
