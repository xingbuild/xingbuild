...
- Task 标题状态的唯一正文是 [`task-onboarding.md`](task-onboarding.md) 第十一节；本文件只保留使用目的，不复制另一套状态规则。所有 task 必须有标题前缀：`🔄`=可继续，`✅`=当前无需 Xing 处理，`⚠️`=缺少 Xing 决策/授权/必要信息而不能继续，`⛔`=技术/网络/外部 task 阻断。
- `⚠️` 不能用于普通风险、正常回传或“等另一个 task”的惯例状态；技术/网络/其他 task 阻断使用 `⛔`，并在检查点写明 `阻断类型` 和恢复 owner，不能让标题状态失真。
