# 已归档｜XBUILD 页面内容结构与内容工作台覆盖方案

状态：已归档；已实现部分由 `v0.26.27`～`v0.26.29` 收口；未实现项拆分到当前活动候选，禁止按本历史文件直接实现。
...
### 6.1 career 交接更正（后续产品输入，不进入当前版本）

career 已确认并更正以下唯一有效规则；它只作为后续产品/Engineering 方案输入，不改变当前 `v0.26.29`：

- 页面只显示两个操作文案：“查看简历”“下载简历”；不额外显示完整简历标题。
- 两个操作统一指向 Xing 控制的同一受保护源 PDF 和同一内容身份。
- 下载文件名在每次点击时按本地实际下载时间动态生成：`金星简历YYYYMMDDHHmm.pdf`。
- 网站不得显示本地源文件名、`copy`、模板名或内部版本号。
- 受保护源文件只能由 Xing 修改、重命名或删除；产品、Engineering、内容与清理流程只能读取、复制为发布资产并核验。

这条更正替代旧候选中“网站显示完整简历标题”和固定下载文件名的建议。进入正式版本前仍需由 `elon` 将其纳入明确的 current/design 范围，再由 `elon engin` 实现和验证。
