...
const treeOid = stagedTreeOid(root); const baseline = captureProtectedFacts(root); const emptyHash = sha256Bytes(Buffer.alloc(0));
...
`);
    ...
    const candidateArchive = execFileSync("git", ["archive", treeOid], { cwd: root, maxBuffer: 64 * 1024 * 1024 }); execFileSync("tar", ["-xf", "-"], { cwd: fixture, input: candidateArchive });
    const fixturePackage = JSON.parse(await readFile(path.join(fixture, "package.json"), "utf8")); fixturePackage.scripts.build = "node fixture-build.mjs"; fixturePackage.scripts.check = "node -e \\\"console.log('fixture check')\\\""; fixturePackage.scripts["release:prepare"] = "node fixture-prepare.mjs"; await writeFile(path.join(fixture, "package.json"), `${JSON.stringify(fixturePackage, null, 2)}\n`); await writeFile(path.join(fixture, "tests/v0281-release-transaction.test.mjs"), "import test from 'node:test'; test('fixture formal check', () => {});\n");
