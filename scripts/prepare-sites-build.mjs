...
const baseSiteArtifact = await createBaseSiteArtifact({ sourceRoot: root, clientDirectory: distClient, productVersion, productCommit: commit, release: { productVersion, productCommit, baseSiteArtifactId }, contentManifest });
...
const release = { ...provisionalRelease, contentManifestHash: hashFile(path.join(distClient, "content-manifest.json")), baseSiteArtifactManifestHash: hashFile(path.join(distClient, "base-site-artifact.json")), clientFiles };
...
for (const file of allFiles.map((entry) => entry.path)) { const absolute = path.join(distClient, file); if (!/\.(?:html|js|css|json|txt|xml|svg)$/.test(absolute)) continue; if (readFileSync(absolute, "utf8").includes(".content-workspace")) throw new Error(`Production build contains workspace path: ${file}`); }
function hashFile(file) { return execFileSync("shasum", ["-a", "256", file], { encoding: "utf8" }).split(/\s+/)[0]; }
